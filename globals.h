...
GLOBAL_VAR float     irisMin             GLOBAL_INIT(0.45); //amw18 try creating two, one for each eye
GLOBAL_VAR float     irisMinL            GLOBAL_INIT(0.45); //amw18 created
GLOBAL_VAR float     irisMinR            GLOBAL_INIT(0.45); //amw18 created
GLOBAL_VAR uint32_t  maxRam              GLOBAL_INIT(3); //amw18 no idea what this is for now lol
...
GLOBAL_VAR int8_t    blinkPin            GLOBAL_INIT(-1); // Manual both-eyes blink pin (-1 = none)
  ...
  GLOBAL_VAR int8_t  boopPin             GLOBAL_INIT(-1); //amw18 changed from A2 to -1 so could use JST input for light
#else
...
//state machine for the simulation eye "mode" and how they respond to light amw18
enum move_state {MOVING, FIXED};//amw18 is the eye moving or fixed? applies to BOTH EYES
enum blink_state {NORMAL, SLOW, UNCONSCIOUS}; //amw18 does the eye blink normally or infrequently? applies to BOTH EYES
enum size_state {MED, BIG, SMALL}; //amw18 is the eye pupil normal, dilated or miotic
enum light_state {BOTH, LEFT, RIGHT, NONE}; //amw18 does the pupil respond to light from both sides, left/right or none?
//make a struct to hold all these values;
typedef struct{
  move_state Move;
  blink_state Blink;
  size_state lSize;
...
extern myConfig        user_loop(myConfig config); //amw18 FIX: send all 4 states for each eye?

// amw18 functions in M4_Eyes.ino
extern void handleLightSensor(myConfig config, uint32_t); //amw18 FIX: send left and right light states??

//amw18 functions in drawEyes.cpp
extern void drawEyes();

//amw18 functions in globals.cpp
extern void initializeDMAS();
extern void loadEyelidGraphics();
extern void loadTextureMaps();
